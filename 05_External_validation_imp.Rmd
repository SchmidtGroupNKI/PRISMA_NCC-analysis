...
# Aims
1. To externally validate BOADICEA (with DCIS excluded from analysis) in imputed data.
...
```{r load}
# Load complete data
  load("WS_04_Descriptive_Imp.RData")
  
  dat <- dat_nodcis
  nrow(dat)
  table(dat$omgedrag, useNA="always")
  dat$bc <- as.numeric(dat$bc)-1
...
# Plot DCA
  ...
  smooth1 <- smooth.spline(NB_df$threshold, NB_df$NB_mod1*1000, spar=0.5)
  lines(smooth1, type = "l", lwd = 3, col=colors[2])
  smooth2 <- smooth.spline(NB_df$threshold, NB_df$NB_mod2*1000, spar=0.5)
  lines(smooth2, type = "l", lwd = 3, col=colors[3])
  smooth3a <- smooth.spline(NB_df$threshold, NB_df$NB_mod3a*1000, spar=0.5)
  lines(smooth3a, type = "l", lwd = 3, col=colors[4])
  smooth3b <- smooth.spline(NB_df$threshold, NB_df$NB_mod3b*1000, spar=0.5)
  lines(smooth3b, type = "l", lwd = 3, col=colors[5])
  smooth4 <- smooth.spline(NB_df$threshold, NB_df$NB_mod4*1000, spar=0.5)
  lines(smooth4, type = "l", lwd = 3, col=colors[6])
  smooth5a <- smooth.spline(NB_df$threshold, NB_df$NB_mod5a*1000, spar=0.5)
  lines(smooth5a, type = "l", lwd = 3, col=colors[7])
  smooth5b <- smooth.spline(NB_df$threshold, NB_df$NB_mod5b*1000, spar=0.5)
...
```{r save}
# Save
save.image("WS_05_Ext_Val_Imp.RData")
```
