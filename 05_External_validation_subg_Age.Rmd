...
# Aims
1. To externally validate BOADICEA (with DCIS excluded from analysis) by Age subgroup.
...
# Validate for Age group <60 {.tabset}
```{r 1_dat}
# Filter on subgroup:
  dat <- dat_nodcis %>% filter(Age<60)
  nrow(dat)
  table(dat$bc)
  ...
  computeCindex_with_ci <- function(predrisk_col) {
    ...
    b1 <- boot(x1, function(u, i)
      computeCindex(
        ...
        tp = tmax,
        weights = dat$wt[i]),
      R = 100) # 1000 bootstrap samples with replacement
...
## Calibration plots
```{r 1_calplots_quantiles, fig.width=5.5, fig.height=8}
# Models 2+ only; Model 1 has too unique breaks for plotting.
predrisk_cols2 <- predrisk_cols[-1]
model_names2 <- model_names[-1]

# Set plot area
par(mfrow = c(ceiling(length(predrisk_cols2) / 2), 2), mar = c(4, 4, 2, 1))

# Apply plotting function to list of predicted risks
for (i in seq_along(predrisk_cols2)) {
  calplot_quintiles(predrisk_cols2[[i]], model_names2[i])
}
...
# Apply plotting function to list of predicted risks
for (i in seq_along(predrisk_cols2)) {
  calplot_deciles(predrisk_cols2[[i]], model_names2[i], limit = 0.07)
}
...
```{r 1_calplots_smooth, fig.width=5.5, fig.height=8}
par(mfrow = c(ceiling(length(predrisk_cols2) / 2), 2), mar = c(4, 4, 2, 1))
...
# Plot DCA
  ...
  text(x = 0.024, y = 7.25, labels = "Chemoprevention\nconsidered", col = "darkgray", cex = 0.8, font = 1)
  text(x = 0.037, y = 7.25, labels = "Chemoprevention\nhighly\nrecommended", col = "darkgray", cex = 0.8, font = 1)
  ...
  computeCindex_with_ci <- function(predrisk_col) {
    ...
    b1 <- boot(x1, function(u, i)
      computeCindex(
        ...
        weights = dat$wt[i]),
      R = 1000) # 1000 bootstrap samples with replacement
    b2_ci <- boot.ci(b1, type = c("norm", "basic", "perc"))
    result <- paste0(cind_est, " (", round(b2_ci$percent[4], 2), ", ", round(b2_ci$percent[5], 2), ")")
    return(result)
  }
...
## Calibration plots
```{r 2_calplots_quantiles, fig.width=5.5, fig.height=8}
# Models 2+ only; Model 1 has too unique breaks for plotting.
predrisk_cols2 <- predrisk_cols[-1]
model_names2 <- model_names[-1]
...
# Apply plotting function to list of predicted risks
for (i in seq_along(predrisk_cols2)) {
  calplot_quintiles(predrisk_cols2[[i]], model_names2[i], limit = 0.10)
}
...
# Apply plotting function to list of predicted risks
for (i in seq_along(predrisk_cols2)) {
  calplot_deciles(predrisk_cols2[[i]], model_names2[i], limit = 0.10)
}
...
## Calibration plots (smooth)
```{r 2_calplots_smooth, fig.width=5.5, fig.height=8}
par(mfrow = c(ceiling(length(predrisk_cols2) / 2), 2), mar = c(4, 4, 2, 1))
